...
public class Part2 {
	...
	private static long getDataLength(String instruction) {
		return getDataLength(instruction, 0, instruction.length());
	}

	private static long getDataLength(String instruction, int startPos, int end) {

		char[] letters = instruction.toCharArray();

		long count = 0;

		for (int i = startPos; i < startPos + end;) {
			...
			if (currentChar == '(') {
				...
				count += repeatNumber * getDataLength(instruction, currentIndex, start);
